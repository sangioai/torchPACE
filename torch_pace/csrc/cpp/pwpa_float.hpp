...
float horner_scheme_fma_cpu(const float* coeffs, int ParamHwDegree, float x, int jump=1) {
...
void evaluate_polynomials_AoS_cpu(const float* x_values, float* y_values, 
...
// Optimized Kernel function to evaluate piecewise polynomials on SoA of coeffs
void evaluate_polynomials_SoA_cpu(const float* x_values, float* y_values, 
                                     const float* coeffs, const float* partition_points, int P, int D, int N) {
    for(int idx=0; idx<N; idx++) {
        float x = x_values[idx];
...
at::Tensor _pwpa_cpu(const at::Tensor& x, const at::Tensor& coeffs, const at::Tensor& partition_points, bool AoS = true) {
  ...
  const float* partition_points_ptr = partition_points_contig.data_ptr<float>();
  float* result_ptr = result.data_ptr<float>();
  int P = partition_points_contig.numel()-1;
  int D = (coeffs_contig.numel() / P) - 1;
  int N = x_contig.numel();
  // run CPU Kernel
  if(AoS)   evaluate_polynomials_AoS_cpu(x_ptr, result_ptr, coeffs_ptr, partition_points_ptr, P, D, N);
  else      evaluate_polynomials_SoA_cpu(x_ptr, result_ptr, coeffs_ptr, partition_points_ptr, P, D, N);
  return result;
}

at::Tensor pwpa_cpu(const at::Tensor& x, const at::Tensor& coeffs, const at::Tensor& partition_points, bool AoS = true) {
  return _pwpa_cpu(x, coeffs, partition_points, AoS);
}
